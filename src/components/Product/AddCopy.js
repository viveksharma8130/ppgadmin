...
const AddProduct = ({ Allcategory }) => {
    ...
    for (const [key, value] of Object.entries(formfield)) {
      data.push(
        <React.Fragment key={key}>
          <AddVariantForm keys={key} value={value} EditVariant={EditVariant} />
        </React.Fragment>
      );
    }
  ...
  const UpdateVariant = (e) => {
    ...
    let filterFormList = variantdata.find((item) => item.id === variantListId);

    console.log("variantData", variantdata);
    console.log("CurrentVariantData", filterFormList);
    console.log("CurrentFormField", formfield);
